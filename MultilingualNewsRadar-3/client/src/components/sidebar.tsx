...
import { X, MessageCircle, Users, BookOpen, Shield, TrendingUp, Video } from "lucide-react";
...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const handleSectionClick = (section: string) => {
    if (section === 'chatbot') {
      ...
      onClose();
    } else if (section === 'video-analyzer') {
      setLocation('/video-analyzer');
      onClose();
    } else {
        ...
        <div className="flex-1 p-4">
          <nav className="space-y-2">
            ...
            >
              ...
              <div>
                <div className="font-medium">Blogs & Insights</div>
                <div className="text-sm text-muted-foreground">Expert analysis and commentary</div>
              </div>
            </Button>
            ...
            >
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">AI-powered content suggestions</div>
              </div>
            </Button>
            ...
            <Button
              variant="ghost"
              className="w-full justify-start text-left h-auto p-3"
              onClick={() => handleSectionClick('video-analyzer')}
            >
              <Video className="h-5 w-5 mr-3" />
              <div>
                <div className="font-medium">Video Analyzer</div>
                <div className="text-sm text-muted-foreground">AI-powered news video summarization</div>
              </div>
            </Button>
          </nav>
        </div>
        ...
        {activeSection === 'blogs' && (
          <div className="flex-1 border-t border-border p-4">
            <div className="text-center text-muted-foreground">
              ...
              <h3 className="text-lg font-medium mb-2">Blogs & Insights</h3>
              <p className="text-sm">Deep-dive analysis, expert opinions, and thought-provoking commentary on the stories that matter.</p>
              <div className="mt-4 space-y-2">
                <div className="text-sm text-left p-3 bg-muted/50 rounded">
                  <strong>Editorial Pieces</strong> - In-depth analysis
                </div>
                <div className="text-sm text-left p-3 bg-muted/50 rounded">
                  <strong>Expert Opinions</strong> - Industry insights
                </div>
                <div className="text-sm text-left p-3 bg-muted/50 rounded">
                  <strong>Weekly Roundups</strong> - Curated highlights
                </div>
              </div>
            </div>
          </div>
