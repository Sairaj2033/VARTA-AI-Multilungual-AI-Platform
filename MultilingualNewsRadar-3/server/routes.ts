...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Initialize with sample data first, then generate new content
  setTimeout(async () => {
    try {
      console.log('Initializing with sample news data...');
      await initializeSampleData();
      
      // Try to generate new articles using Gemini AI
      try {
        await newsService.generateAndRefresh();
      } catch (error) {
        console.log('AI generation failed, using sample data only');
      }
    } catch (error) {
